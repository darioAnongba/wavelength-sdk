...
SDK codes include `runtime_not_ready`, `asset_load_failed`,
`asset_integrity_failed`, `worker_error`, `wallet_locked`, and
`runtime_lock_unavailable` (the browser refused or cancelled the runtime lock
request itself, which says nothing about another tab; offer a plain retry).
