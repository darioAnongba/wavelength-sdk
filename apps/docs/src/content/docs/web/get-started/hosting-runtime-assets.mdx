...
Before executing the wasm binary and the two bootstrap scripts it runs
(`wasm_exec.js` and `sqlite-bridge.js`), the web transport verifies their
fetched bytes against SHA-256 digests pinned in the npm package for the
