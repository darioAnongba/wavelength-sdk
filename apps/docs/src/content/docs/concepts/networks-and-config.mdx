...
| Network | Typical use |
|---------|-------------|
| `'signet'` | Default for docs live embeds and the demo app; public Lightning Labs test infrastructure. Get test coins from the [signet faucet](https://robinet.eldamar.icu/). |
...
not enough to connect. Access is keyed to your client's identity, its
mailbox ID, which `wavecli getinfo` reports as `identity_pubkey`.
...
const config: RuntimeConfig = {
  ...
  walletBlockHeadersSource: 'https://headers.example.com/block-headers.bin',
  walletFilterHeadersSource: 'https://headers.example.com/filter-headers.bin',
...
- `maxOperatorFeeSat`, `maxPaymentCLTV`, `signingWorkers`, and `bufferSize` are
  numeric runtime limits. They must be nonnegative safe integers.
- `maxPaymentCLTV` is the largest total Lightning payment CLTV, in blocks,
  that automatic VTXO maintenance reserves. Leave it unset to use the daemon
  default. The mobile facade distinguishes an explicit `0` from an omitted
  value and uses it to disable the extra reserve. The value must also fit in a
...
and invalid numeric values before it starts the daemon. Some daemon-only
options (such as the LND wallet backend) are deliberately not exposed through
`RuntimeConfig`.
