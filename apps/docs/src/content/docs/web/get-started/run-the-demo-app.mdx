...
description: How to clone, install, and run the Wavelength demo app locally against signet, testnet, or a custom backend.
---
...
The demo depends on the workspace packages `@lightninglabs/wavelength-web` and
...
```bash title="Terminal"
pnpm --filter web-wallet-demo run dev:local
```
...
Use `pnpm --filter web-wallet-demo run build:local && pnpm --filter web-wallet-demo preview`
...
[web guides](/guides/create-a-wallet/) for end-to-end wallet flows built
on top of this demo.
