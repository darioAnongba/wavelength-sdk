...
| `--request-json` | (none) | Raw JSON request payload that maps directly to the RPC request proto. When set, it supplies the complete RPC request. Do not combine it with command-specific request inputs; command-local output and post-processing flags may still apply. |
...
drive the RPC request from a single serialized payload instead of assembling
request flags. Do not combine it with command-specific request inputs. For
example, `bakemacaroon` rejects positional permissions and `--root-key-id`
when `--request-json` is set, while output and post-processing flags such as
`--save-to` and `--valid-for` still apply to the returned macaroon.
`--request-json` is distinct from the `--json` output selector.
...
| [`bakemacaroon`](/cli/bakemacaroon/) | Create a daemon macaroon with scoped permissions. |
